...
""")
  ...
  List<AnnotationBatch> findLatestPerTaskByJobId(
      @Param("jobId") String jobId, @Param("page") int page, @Param("page_size") int pageSize);

  @Query(
      nativeQuery = true,
      value =
          """
						    select count(*) from annotation_batch a
						    inner join task t on t.id = a.task_id
						    inner join job j on j.id = t.job_id
						    inner join (
						        select a1.task_id, max(a1.creation_timestamp) as latest_creation_timestamp
						    ...
						    where j.id = :jobId
						""")
  int countLatestAnnotationBatchBy(@Param("jobId") String jobId);
}
